  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: 400 }}>
        <div style={{ fontSize: 14, color: '#64748b' }}>Cargando espacios...</div>
      </div>
    );
  }
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(290px, 1fr))', gap: 16 }}>
        {filtered.length === 0 && (
          <div style={{ gridColumn: '1 / -1', ...card, padding: '40px 24px', textAlign: 'center' }}>
            <div style={{ fontSize: 32, fontWeight: 300, color: '#cbd5e1', marginBottom: 8 }}>{'{}'}</div>
            <div style={{ fontSize: 14, fontWeight: 600, color: '#94a3b8' }}>No se encontraron espacios</div>
            <div style={{ fontSize: 13, color: '#cbd5e1', marginTop: 4 }}>Crea un nuevo espacio con el botón "Nuevo Espacio".</div>
          </div>
        )}
        {filtered.map(room => {
